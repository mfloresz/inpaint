  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={[
          'flex-shrink-0',
          'mt-4 bg-card border border-border p-3 rounded-lg shadow-sm',
          'flex items-left w-full max-w-4xl',
          'space-y-0 flex-row space-x-5',
          'scrollbar-thin scrollbar-thumb-muted-foreground/20 scrollbar-track-transparent overflow-x-scroll',
        ].join(' ')}
      >
        {History}
      </div>
      ...
      <div
        className={[
          'flex-shrink-0',
          'bg-card/80 backdrop-blur-md rounded-xl border border-border shadow-md p-4 transition duration-200 ease-in-out',
          'flex items-center w-full max-w-4xl py-6 mb-4 justify-between',
          'flex-col space-y-4 sm:space-y-0 sm:flex-row sm:space-x-8',
        ].join(' ')}
      >
        {renders.length > 0 && (
          <Button
